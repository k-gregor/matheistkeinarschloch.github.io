...
links.Graph3d = function (container) {
	...
	this.integralarea = undefined;

	this.withConstraint = false;
...
links.Graph3d.prototype.draw = function(data, options) {
	...
	if (options !== undefined) {
		...
		if(options.integralarea !== undefined) this.integralarea = options.integralarea;
		
		if(options.withConstraint !== undefined) this.withConstraint = options.withConstraint;
	}
...
links.Graph3d.prototype._getStyleNumber = function(styleName) {
	...
	case "vectors":		return links.Graph3d.STYLE.VECTORS;
...
links.Graph3d.prototype._determineColumnIndexes = function(data, style) {
	if (this.style === links.Graph3d.STYLE.DOT ||
			...
			this.style === links.Graph3d.STYLE.BAR || this.style === links.Graph3d.STYLE.VECTORS) {
		
		console.log("FIRST IF, STYLE IS" + this.style);
...
links.Graph3d.prototype._getDataPoints = function (data) {
	return _getDataPoints(data, false);
};
 ...
 */
links.Graph3d.prototype._getDataPoints = function (data, isConstraint) {
	...
	if (this.style === links.Graph3d.STYLE.GRID ||
		...
		//TODO TODO is mostly undefined, but should be false in that case
		if(isConstraint !== true) {
			...
			for (x = 0; x < dataMatrix.length; x++) {
				for (y = 0; y < dataMatrix[x].length; y++) {
					if (dataMatrix[x][y]) {
						dataMatrix[x][y].pointRight = (x < dataMatrix.length-1 && y>0) ? dataMatrix[x+1][y-1] : undefined;
						dataMatrix[x][y].pointTop   = (x>0 && y < dataMatrix[x-1].length-1) ? dataMatrix[x-1][y+1] : undefined;
						dataMatrix[x][y].pointCross = undefined; //not a grid!
					}
				}
			}
...
links.Graph3d.prototype._readData = function(data) {
	...
	if (this.dataFilter) {
		...
		this.dataPoints = this._getDataPoints(this.dataTable, false);

		if(this.withConstraint == true){
			var constraintData = new google.visualization.DataTable();
			constraintData.addColumn('number', 'x');
			constraintData.addColumn('number', 'y');
			constraintData.addColumn('number', 'z');

			for(var x=this.xMin; x<=this.xMax; x+=1){
				constraintData.addRow([x, (this.constraint_m*x+this.constraint_n), 13.333]);
			}
			var constraintDataPoints = this._getDataPoints(constraintData, true);

			for(var i=0;i<constraintDataPoints.length; i++){
				constraintDataPoints[i].isConstraint = true;
				this.dataPoints.push(constraintDataPoints[i]);
...
links.Graph3d.prototype.redraw = function(data) {
	...
	else if (this.style === links.Graph3d.STYLE.VECTORS){
		this._redrawDataVectors();
	}
...
links.Graph3d.prototype._redrawAxis = function() {
	...
	if(this.withConstraint){
		ctx.lineWidth = 2;
		xMin2d = this._convert3Dto2D(new links.Point3d(this.xMin, this.constraint_m*this.xMin+this.constraint_n, this.zMin));
		xMax2d = this._convert3Dto2D(new links.Point3d(this.xMax, this.constraint_m*this.xMax+this.constraint_n, this.zMin));
		ctx.strokeStyle = "red";
		ctx.beginPath();
		ctx.moveTo(xMin2d.x, xMin2d.y);
		ctx.lineTo(xMax2d.x, xMax2d.y);
	...
	if(this.integralarea !== undefined){
	    ctx.lineWidth = 2;
		ctx.strokeStyle = "red";
		ctx.beginPath();
		p = this._convert3Dto2D(new links.Point3d(this.integralarea[0][0], this.integralarea[0][1], this.zMin));
		ctx.moveTo(p.x, p.y);
		for(i=1; i<this.integralarea.length; i++){
		    p = this._convert3Dto2D(new links.Point3d(this.integralarea[i][0], this.integralarea[i][1], this.zMin));
		    ctx.lineTo(p.x, p.y);
		}
...
links.Graph3d.prototype._hsv2rgb = function(H, S, V) {
    ...
    if(this.integralarea !== undefined)
    	return "RGBA(" + parseInt(R*255) + "," + parseInt(G*255) + "," + parseInt(B*255) + ", " + this.transparency + ")";
    else
 ...
 * This function can be used when the style is "grid" //surface!
 */
links.Graph3d.prototype._redrawDataGrid = function() {

	var canvas = this.frame.canvas,
	ctx = canvas.getContext("2d"),
	point, right, top, cross, isConstraint,
	i,
	...
	for (i = 0; i < this.dataPoints.length; i++) {
		...
		//gibt distanz vom punkt zur kamera wieder (aber in "echter" x,y,z Richtung, heiÃŸt: z geht in die Kamera rein).
		if(this.dataPoints[i].isConstraint !== true){
			...
			//MAKES ALL CONSTRAINT POINTS VISIBLE AT ALL TIMES
			this.dataPoints[i].dist = -1000;
		}
	}

	// sort the points on depth of their (x,y) position (not on z), highest distance first
	//a.dist < b.dist <=> b-a<0 --> b kommt vor a in der graphik
	...
	if (this.style === links.Graph3d.STYLE.SURFACE) {
		for (i = 0; i < this.dataPoints.length; i++) {
			...
			isConstraint = this.dataPoints[i].isConstraint;

			if (point !== undefined && right !== undefined && top !== undefined && (cross !== undefined || isConstraint == true)) {
		...
		for (i = 0; i < currentDataPoints.length; i++) {
			point = currentDataPoints[i];
			right = currentDataPoints[i].pointRight;
			top   = currentDataPoints[i].pointTop;
...
links.Graph3d.prototype._redrawDataVectors = function() {
	...
	point = new links.Point3d(0, 0, 0);
	var trans = this._convertPointToTranslation(point);
	var screenZero = this._convertTranslationToScreen(trans);
	
	ctx.lineWidth = 2;        // TODO: make customizable
	...
	for (i = 0; i < this.dataPoints.length; i++) {
		...
		ctx.moveTo(screenZero.x, screenZero.y);
		point = this.dataPoints[i].point;
		var trans = this._convertPointToTranslation(point);
		var screen = this._convertTranslationToScreen(trans);
		this.dataPoints[i].trans = trans;
		this.dataPoints[i].screen = screen;
		...
		var point2 = new links.Point3d(point.x - 0.1*point.x - 0.1, point.y - 0.1*point.y, point.z - 0.1*point.z);
		var screen2 = this._convertTranslationToScreen(this._convertPointToTranslation(point2));
...
links.Graph3d.prototype._onMouseMove = function (event) {
	...
	var horizontalNew = this.startArmRotation.horizontal + diffX / 200;Math.min(this.startArmRotation.horizontal + diffX / 200, 2.33);

	if(this.withConstraint){
		horizontalNew = Math.max(horizontalNew, -0.8);
		horizontalNew = Math.min(horizontalNew, 2.33);
	}
...
links.Graph3d.prototype._onTooltip = function (event) {
	...
	elementOffsetTop = $(mygraph).offset().top,
	distanceToTop      = (elementOffsetTop - scrollTop);
	var scrollLeft     = $(window).scrollLeft(),
	elementOffsetLeft = $(mygraph).offset().left,
	distanceToLeft      = (elementOffsetLeft - scrollLeft);

	var mouseX = links.getMouseX(event) - distanceToLeft;
	var mouseY = links.getMouseY(event) - distanceToTop;
...
links.Graph3d.prototype._showTooltip = function (dataPoint) {
	...
	if (typeof this.showTooltip === 'function') {
		content.innerHTML = this.showTooltip(dataPoint);
	}
	else {
...
links.Point3d.multiply = function(a, scalar) {
	var mult = new links.Point3d();
	mult.x = a.x * scalar;
	mult.y = a.y * scalar;
	mult.z = a.z * scalar;
	return mult;
...
links.Point3d.dotProduct = function(a, b) {
	return a.x * b.x + a.y * b.y + a.z * b.z;
};

links.Point3d.normOf = function(a){
	return Math.sqrt(
			a.x * a.x +
			a.y * a.y +
			a.z * a.z
